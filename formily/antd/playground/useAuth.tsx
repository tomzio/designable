...
export const AuthProvider = (props: AuthWrapperProps) => {
  ...
  const [accessToken, setAccessToken] = useState<string | undefined>(sessionToken)
...
export const useAuth = () => {
  ...
  const quickLogin = async (values: any): Promise<any> => {
    const url = `/his/app/test/getauthtoken?${qs.stringify(values)}`
    const token = await fetch(url).then((res) => res.text())
    console.log('token: ', token)

    const session = await fetch(`/his/app/auth/index?key=${token}`, { method: 'POST' })
      .then((res) => res.json())
      .then((res) => {
        const { data } = res
    ...
    if (token && session) {
      context.setAccessToken(session.sessionToken)
